...
export function mergeSpeakerProfileVectors(groups, options = {}) {
  ...
  return {
    ...
    consistency,
    matchThreshold: Number(options.matchThreshold) || DEFAULT_MATCH_THRESHOLD
  };
}
...
export class SpeakerEmbeddingModel {
  constructor(session, options = {}) {
    ...
    this.matchThreshold = Number(options.matchThreshold) || DEFAULT_MATCH_THRESHOLD;
    this.profileConsistencyThreshold = Number(options.profileConsistencyThreshold) || 0.58;
  }
  ...
  async createProfile(pcm) {
    ...
    const consistency = profileConsistency(exemplars);
    if (exemplars.length >= 2 && consistency < this.profileConsistencyThreshold) {
...
export class RemoteSpeakerEmbeddingModel extends SpeakerEmbeddingModel {
  constructor({
    origin,
    token,
    dimensions = 512,
    matchThreshold = 0.6,
    profileConsistencyThreshold = 0.45,
    fetchImpl = fetch
  }) {
    super(null, { matchThreshold, profileConsistencyThreshold });
  ...
  async embed(pcm) {
    ...
    const response = await this.fetchImpl(`${this.origin}/v1/embeddings`, {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      throw new Error(`원격 화자 모델 요청 실패: HTTP ${response.status} ${result?.error || ""}`.trim());
    }
    if (!Array.isArray(result.embedding) || result.embedding.length !== this.dimensions
      || result.embedding.some((value) => !Number.isFinite(Number(value)))) {
      throw new Error("원격 화자 모델이 유효하지 않은 임베딩을 반환했습니다.");
    }
    return normalize(result.embedding.map(Number));
  }
}

let modelPromise;

export function getSpeakerEmbeddingModel(cacheDirectory, modelPath, remoteOptions = {}) {
  if (!modelPromise) {
    const pending = remoteOptions.origin
      ? (async () => {
        const token = String(remoteOptions.token || await readFile(remoteOptions.tokenFile, "utf8")).trim();
        if (token.length < 32) throw new Error("원격 화자 모델 토큰이 유효하지 않습니다.");
        return new RemoteSpeakerEmbeddingModel({ ...remoteOptions, token });
      })()
      : SpeakerEmbeddingModel.create(cacheDirectory, modelPath);
...
const remoteSpeakerOrigin = String(process.env.SPEAKER_INFERENCE_ORIGIN || "").trim();
const remoteSpeakerModelId = String(process.env.SPEAKER_INFERENCE_MODEL_ID
  || "sherpa-onnx/3dspeaker-eres2net-base-200k").trim();
const remoteSpeakerDimensions = Math.max(1, Number(process.env.SPEAKER_INFERENCE_DIMENSIONS) || 512);
const remoteSpeakerMatchThreshold = Number(process.env.SPEAKER_MATCH_THRESHOLD) || 0.6;
const remoteSpeakerMatchMargin = Number(process.env.SPEAKER_MATCH_MARGIN) || 0.08;
...
export const speakerInferenceInfo = Object.freeze({
  ...
  realtimeMaximumEmbeddings: remoteSpeakerOrigin ? 1 : 2,
  evaluationMaximumEmbeddings: 3,
  aggregation: "median"
});

export const speakerModelInfo = {
  id: remoteSpeakerOrigin ? remoteSpeakerModelId : MODEL_ID,
  url: remoteSpeakerOrigin || MODEL_URL,
  sampleRate: SAMPLE_RATE,
  dimensions: remoteSpeakerOrigin ? remoteSpeakerDimensions : 256,
  defaultMatchThreshold: remoteSpeakerOrigin ? remoteSpeakerMatchThreshold : DEFAULT_MATCH_THRESHOLD,
  defaultMatchMargin: remoteSpeakerOrigin ? remoteSpeakerMatchMargin : 0.05
};
