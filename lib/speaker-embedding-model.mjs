...
export class RemoteSpeakerEmbeddingModel extends SpeakerEmbeddingModel {
  constructor({ origin, token, fetchImpl = fetch }) {
    super(null, { matchThreshold: 0.6, profileConsistencyThreshold: 0.45 });
  ...
  async embed(pcm) {
    ...
    if (!Array.isArray(result.embedding) || result.embedding.length !== 512
...
export const speakerModelInfo = {
  id: remoteSpeakerOrigin ? "sherpa-onnx/3dspeaker-eres2net-base-200k" : MODEL_ID,
  url: remoteSpeakerOrigin || MODEL_URL,
  sampleRate: SAMPLE_RATE,
  dimensions: remoteSpeakerOrigin ? 512 : 256,
  defaultMatchThreshold: remoteSpeakerOrigin ? 0.6 : DEFAULT_MATCH_THRESHOLD,
  defaultMatchMargin: remoteSpeakerOrigin ? 0.08 : 0.05
};
