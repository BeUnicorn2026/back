  ...
  properties: {
    ...
    terms: {
      type: "array", maxItems: 30, items: {
        type: "object", additionalProperties: false,
        required: ["term", "definition", "explanation", "evidenceSegmentIndex"],
        properties: {
...
export function transcriptHash(segments) {
  ...
  return createHash("sha256").update("meeting-intelligence-v2\0").update(JSON.stringify(normalized)).digest("hex");
}
...
function normalizeResult(raw, segments) {
  ...
  const terms = (Array.isArray(raw?.terms) ? raw.terms : []).map((term) => {
    ...
    const explanation = cleanText(term.explanation || term.personalizedExplanation, 700);
    return {
      term: name,
  ...
  const actions = (Array.isArray(raw?.actions) ? raw.actions : []).map((action, index) => {
    ...
    return {
      ...
      text: evidence.text,
      owner: actionOwnerFromEvidence(evidence.text, evidence.speaker),
      due: actionDueFromEvidence(evidence.text),
...
function localAnalysis(segments) {
  ...
  return normalizeResult(raw, segments);
}
...
export class MeetingIntelligenceService {
  ...
  async analyze(meeting) {
    ...
    try {
      response = await this.fetch("https://api.openai.com/v1/responses", {
        ...
        body: JSON.stringify({
          ...
          instructions: "한국어 회의 분석기다. 제공된 실제 발화만 근거로 주제, 요약, 전문용어, 실행 항목을 만든다. 모든 참여자가 공유하는 결과이므로 사용자 역할이나 지식 수준을 추정하지 말고, 전문용어 explanation은 중립적이고 정확한 기본 설명으로 작성한다. 발화에 없는 사실·담당자·기한을 만들지 않는다. segmentIndexes와 evidenceSegmentIndex는 입력의 대괄호 번호만 사용한다.",
          input: JSON.stringify({ transcript: selected }),
    ...
    const result = normalizeResult(JSON.parse(text), segments);
    return {
      ...result,
      source: "openai",
      model: this.model,
