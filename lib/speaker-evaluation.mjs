...
export function calibrateSpeakerThreshold(trials, speakers, options = {}) {
  ...
  const marginValues = options.margin == null
    ? numericRange(Number(options.minimumMargin ?? 0.02), Number(options.maximumMargin ?? 0.12), Number(options.marginStep ?? 0.01))
  ...
  for (const margin of marginValues) {
    for (let threshold = minimum; threshold <= maximum + Number.EPSILON; threshold += step) {
      ...
      const accuracy = report.rates.overallDecisionAccuracy || 0;
      if (!best || score < best.objective
        || (score === best.objective && accuracy > best.rates.overallDecisionAccuracy)
        || (score === best.objective && accuracy === best.rates.overallDecisionAccuracy && rounded > best.threshold)) {
...
function numericRange(minimum, maximum, step) {
  if (!Number.isFinite(minimum) || !Number.isFinite(maximum) || !Number.isFinite(step) || step <= 0 || maximum < minimum) {
    throw new Error("화자 보정 범위가 올바르지 않습니다.");
  }
  const values = [];
  for (let value = minimum; value <= maximum + Number.EPSILON; value += step) {
