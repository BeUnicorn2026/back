...
export function chooseKnownSpeaker(scores, speakers, options = {}) {
  if (!Array.isArray(scores) || !scores.length || scores.length !== speakers.length) return null;
  ...
  if (!best || best.score < bestThreshold) return null;
  if (second && best.score - second.score < margin) return null;

  return { id: best.speaker.id, name: best.speaker.name, score: best.score };
}
