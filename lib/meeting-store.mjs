...
export class MeetingStore {
  ...
  async #initialize() {
    ...
    if (!segmentColumns.some(({ name }) => name === "user_id")) {
      this.database.exec("ALTER TABLE meeting_segments ADD COLUMN user_id TEXT");
    }
    if (!segmentColumns.some(({ name }) => name === "speaker_profile_id")) {
      this.database.exec("ALTER TABLE meeting_segments ADD COLUMN speaker_profile_id TEXT");
    }
    if (!segmentColumns.some(({ name }) => name === "sequence")) {
      this.database.exec("ALTER TABLE meeting_segments ADD COLUMN sequence INTEGER");
      this.database.exec("UPDATE meeting_segments SET sequence = position WHERE sequence IS NULL");
    }
    ...
    if (!meetingColumns.some(({ name }) => name === "room_id")) {
      this.database.exec("ALTER TABLE meetings ADD COLUMN room_id TEXT");
    }
    ...
    this.database.exec(`CREATE UNIQUE INDEX IF NOT EXISTS meetings_active_room_idx
  ...
  #replaceSegments(meetingId, segments) {
    ...
    const insert = this.database.prepare(`INSERT INTO meeting_segments
      (meeting_id, position, id, speaker, known, corrected, transcript_corrected, confidence, transcript_confidence,
        source_speaker, user_id, speaker_profile_id, sequence, start, end, text)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
    segments.forEach((segment, position) => insert.run(meetingId, position, segment.id, segment.speaker,
      ...
      segment.confidence, segment.transcriptConfidence, segment.sourceSpeaker, segment.userId,
      segment.speakerProfileId, segment.sequence, segment.start, segment.end, segment.text));
  }
  ...
  async countSince(organizationId, startedAt) {
    const database = await this.#ready();
    const row = database.prepare("SELECT COUNT(*) AS count FROM meetings WHERE organization_id = ? AND started_at >= ?")
      .get(organizationId, String(startedAt || ""));
  ...
  async bindRoomMeeting({ organizationId, createdBy, roomId, language = "ko", title = "", requestedMeetingId = "" }) {
    ...
    return runTransaction(database, () => {
      const room = database.prepare(`SELECT id FROM rooms
        WHERE id = ? AND organization_id = ? AND status = 'active'`).get(roomId, organizationId);
      if (!room) return null;
      if (requestedMeetingId) {
        const requested = database.prepare(`SELECT id FROM meetings
          WHERE id = ? AND organization_id = ? AND room_id = ? AND status = 'recording'`)
          .get(requestedMeetingId, organizationId, roomId);
        return requested ? summarize(this.#meetingFromId(requested.id, organizationId)) : null;
      }
      const existing = database.prepare(`SELECT id FROM meetings
        WHERE organization_id = ? AND room_id = ? AND status = 'recording'`)
        .get(organizationId, roomId);
      ...
      database.prepare(`INSERT INTO meetings
        ...
        VALUES (?, ?, ?, ?, ?, ?, 'live', 'speaker', 'recording', 0, ?, NULL, ?)`)
        .run(id, organizationId, createdBy, roomId, cleanText(title, 120) || "새 회의",
          cleanText(language, 12) || "ko", now, now);
  ...
  async create({ organizationId, createdBy, roomId = null, language = "ko", source = "live", mode = "speaker", title = "" }) {
    ...
    return runTransaction(database, () => {
      ...
      if (sanitizedRoomId) {
        const existing = database.prepare(`SELECT id FROM meetings
          WHERE organization_id = ? AND room_id = ? AND status = 'recording'`)
          .get(organizationId, sanitizedRoomId);
        if (existing) return summarize(this.#meetingFromId(existing.id, organizationId));
      }
      const now = new Date().toISOString();
      const id = randomUUID();
      database.prepare(`INSERT INTO meetings
        ...
        .run(id, organizationId, createdBy, sanitizedRoomId, cleanText(title, 120) || "새 회의",
          cleanText(language, 12) || "ko", allowedSources.has(source) ? source : "live",
          allowedModes.has(mode) ? mode : "speaker", now, now);
      return summarize(this.#meetingFromId(id, organizationId));
    });
  }
  ...
  async createCompleted({ organizationId, createdBy, roomId = null, language = "ko", source = "upload", mode = "speaker", title = "", segments = [], duration = 0, importKey = null }) {
    ...
    return runTransaction(database, () => {
      ...
      database.prepare(`INSERT INTO meetings
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'completed', ?, ?, ?, ?, ?)`)
        .run(id, organizationId, createdBy, cleanText(roomId, 120) || null, sanitizedTitle,
          cleanText(language, 12) || "ko", allowedSources.has(source) ? source : "upload",
          allowedModes.has(mode) ? mode : "speaker", sanitizedDuration, now, now, now, sanitizedImportKey);
  ...
  async appendAcceptedSegment(meetingId, organizationId, segment) {
    const database = await this.#ready();
    return runTransaction(database, () => {
      const current = this.#meetingFromId(meetingId, organizationId);
      if (!current || current.status !== "recording") return null;
      const sanitized = cleanSegments([{ ...segment, sequence: 0 }])[0];
      if (!sanitized) throw new Error("저장할 대화 내용이 필요합니다.");
      const row = database.prepare(`SELECT COALESCE(MAX(sequence), -1) + 1 AS sequence,
        COALESCE(MAX(position), -1) + 1 AS position FROM meeting_segments WHERE meeting_id = ?`).get(meetingId);
      const sequence = Number(row.sequence);
      database.prepare(`INSERT INTO meeting_segments
        (meeting_id, position, id, speaker, known, corrected, transcript_corrected, confidence, transcript_confidence,
          source_speaker, user_id, speaker_profile_id, sequence, start, end, text)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
        .run(meetingId, Number(row.position), sanitized.id, sanitized.speaker, sanitized.known ? 1 : 0,
          sanitized.corrected ? 1 : 0, sanitized.transcriptCorrected ? 1 : 0, sanitized.confidence,
          sanitized.transcriptConfidence, sanitized.sourceSpeaker, sanitized.userId, sanitized.speakerProfileId,
          sequence, sanitized.start, sanitized.end, sanitized.text);
      database.prepare("UPDATE meetings SET updated_at = ? WHERE id = ? AND organization_id = ?")
        .run(new Date().toISOString(), meetingId, organizationId);
  ...
  async update(id, organizationId, changes = {}) {
    ...
    return runTransaction(database, () => {
      ...
      if (replaceSegments) this.#replaceSegments(id, segments);
      return summarize(this.#meetingFromId(id, organizationId));
    });
  }
  ...
  async remove(id, organizationId) {
    const database = await this.#ready();
    return runTransaction(database, () => {
      const result = database.prepare("DELETE FROM meetings WHERE id = ? AND organization_id = ?").run(id, organizationId);
  ...
  async saveIntelligence({ meetingId, organizationId, transcriptHash, source, model, result }) {
    ...
    return runTransaction(database, () => {
      ...
      if (analyzedTitle) {
        database.prepare("UPDATE meetings SET title = ?, updated_at = ? WHERE id = ? AND organization_id = ?")
          .run(analyzedTitle, now, meetingId, organizationId);
      }
      return {
        ...result,
        source,
