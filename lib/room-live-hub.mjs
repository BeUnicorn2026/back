...
import { createNoopTermLiveBridge } from "./term-live-bridge.mjs";
...
export class RoomLiveHub {
  constructor({
    ...
    this.termBridgeFactory = termBridgeFactory;
  ...
  async acquire({
    ...
    if (participant) session.participants.set(client, participant);
    try {
      await session.ready;
  ...
  #createSession({
    ...
    const session = {
      ...
      // 개인화 푸시용 참가자 스냅샷: 각 참가자의 자기소개와 함께, 그 사람의 소켓
      // 하나로만 보내는 send를 제공한다 (broadcast와 달리 참가자별 내용이 다르다).
      listParticipants() {
        const listed = [];
        for (const [member, participant] of this.participants) {
          listed.push({
            ...participant,
            send: (payload) => {
              if (member.readyState === WebSocket.OPEN) member.send(JSON.stringify(payload));
            }
          });
        }
        return listed;
      ...
      acceptAccepted(segment, shouldBroadcast) {
        ...
        try {
          if (shouldBroadcast) this.termBridge.handleFinalSegment(segment);
        } catch { /* Persistence is authoritative. */ }
        if (shouldBroadcast) {
    ...
    session.termBridge = this.termBridgeFactory
      ? this.termBridgeFactory({
        participants: () => session.listParticipants(),
        meetingTopic,
        log
      })
      : createNoopTermLiveBridge();
  ...
  #cleanup(session) {
    ...
    session.closing = (async () => {
      try {
        ...
        try { await session.termBridge.finalize(); } catch { /* best effort */ }
        ...
        try { await session.termBridge.dispose(); } catch { /* best effort */ }
