...
async function deleteObsoletePayloads(client, paths, token) {
  let failure;
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      await client.del(paths, { token });
      return;
    } catch (error) {
      failure = error;
    }
  }
  const error = new Error("교체된 이전 생체정보 Blob을 정리하지 못했습니다.", { cause: failure });
...
export class BlobSpeakerStore {
  ...
  async get(id, organizationId = null) {
    try {
      const metadata = JSON.parse((await this.#read(id, "speaker.json")).toString("utf8"));
      if (metadata.id !== id || (organizationId && metadata.organizationId !== organizationId)) return null;
      return { ...metadata, encryptedAtRest: true };
    } catch (error) {
  ...
  async replace(metadata, profile, referenceWav, organizationId) {
    const current = await this.get(metadata.id, organizationId);
    ...
    if (previousFiles.length) await deleteObsoletePayloads(this.client, previousFiles, this.token);
    return storedMetadata;
  }
  ...
  async remove(id, organizationId = null) {
    if (organizationId && !(await this.get(id, organizationId))) return false;
    const page = await this.client.list({
