...
export class PostgresKnowledgeStore {
  ...
  async #initialize() {
    await this.database.query(`
      ...
      ALTER TABLE knowledge_explanations ADD COLUMN IF NOT EXISTS answered_choice_index INTEGER;
      ALTER TABLE knowledge_explanations ADD COLUMN IF NOT EXISTS answered_at TEXT;
    `);
  }
  ...
  async claimExplanationAnswer(userId, cacheKey, choiceIndex) {
    ...
    const now = new Date().toISOString();
    return (await this.database.query(`UPDATE knowledge_explanations
      SET answered_choice_index = $1, answered_at = $2, updated_at = $2
      WHERE user_id = $3 AND cache_key = $4 AND answered_choice_index IS NULL`,
    [safeChoice, now, userId, clean(cacheKey, 64)])).rowCount === 1;
  }
