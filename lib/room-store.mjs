...
export class RoomStore {
  ...
  async create(input) {
    ...
    return runTransaction(database, () => {
      ...
      if (occupied?.created_by === createdBy) return roomFromRow(occupied, { includeAccessCode: true });
      ...
      for (let attempt = 0; attempt < this.collisionRetries; attempt += 1) {
        ...
        if (occupiedAfterCollision?.created_by === createdBy) {
          return roomFromRow(occupiedAfterCollision, { includeAccessCode: true });
        }
        if (occupiedAfterCollision) throw new RoomStoreError("ROOM_EXISTS", "이 조직에서 사용 중인 방 코드입니다.");
      }
