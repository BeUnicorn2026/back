...
export function normalizeRoomCreateInput({ organizationId, createdBy, command, idempotencyKey } = {}) {
  const normalized = {
  ...
  if (normalized.command !== "ROOM") {
    throw new RoomStoreError("ROOM_COMMAND_REQUIRED", "방은 ROOM을 입력한 경우에만 만들 수 있습니다.");
  }
...
export class RoomStore {
    ...
    return runTransaction(database, () => {
      ...
      for (let attempt = 0; attempt < this.collisionRetries; attempt += 1) {
        ...
        if (!ROOM_PATTERN.test(room)) {
          throw new RoomStoreError("ROOM_GENERATOR_INVALID", "생성된 방 번호 형식이 올바르지 않습니다.");
        }
        ...
        const occupiedAfterCollision = database.prepare(
          "SELECT * FROM rooms WHERE organization_id = ? AND room = ? AND status = 'active'"
        ).get(organizationId, room);
        if (occupiedAfterCollision) continue;
      }
  ...
  async join({ organizationId, userId, accessCode, room } = {}) {
    ...
    if (!normalizedAccessCode && !ROOM_PATTERN.test(normalizedRoom)) {
    ...
    return runTransaction(database, () => {
      ...
      const row = database.prepare(`SELECT * FROM rooms
        WHERE organization_id = ? AND ${lookupColumn} = ? AND status = 'active' LIMIT 1`)
        .get(normalizedOrganizationId, lookupValue);
      if (!row) {
        const closed = database.prepare(`SELECT 1 FROM rooms
          WHERE organization_id = ? AND ${lookupColumn} = ? AND status = 'closed' LIMIT 1`)
          .get(normalizedOrganizationId, lookupValue);
