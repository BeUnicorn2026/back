...
export function createTermLiveBridge({
  ...
  filter,
  participants,
  // meetingTopic은 논문 원문 프롬프트(A.1)에 입력 슬롯이 없어 현재 미사용이지만,
  // 호출부 배선을 유지하기 위해 계속 받는다.
  meetingTopic = "",
  log = () => {},
  minSentences = 2,
  maxChunkChars = 280
  ...
  async function processChunk(chunk) {
    // 본인이 말한 내용의 용어 카드는 본인에게 보여줄 이유가 없다. 이 청크를 받을
    // 사람이 화자 자신뿐이면 Stage 1 호출조차 하지 않는다 (API 요청 절약).
    const snapshot = listParticipants();
    const receivers = snapshot.filter((participant) =>
      !participant.userId || !chunk.speakerUserIds.includes(participant.userId));
    ...
    await Promise.all(receivers.map(async (participant) => {
      ...
      try {
        const filtered = await filter.familiarTerms({
          ...
          candidateTerms: fresh.map(({ term, definition }) => ({ term, definition })),
          knownTerms: participant.knownTerms || []
        });
  ...
  function flushPending() {
    ...
    const chunk = {
      ...
      speakerUserIds: [...pending.userIds]
    };
    pending = null;
    return chunk;
  }
  ...
  function handleFinalSegment(segment) {
    ...
    try {
      ...
      if (!pending) {
        pending = {
          texts: [], sentences: 0, chars: 0, start, end,
          speaker: String(segment?.speaker || "").trim() || "화자",
          userIds: new Set()
        };
      }
      pending.texts.push(text);
      // 방 회의 세그먼트에는 화자 본인의 userId가 실린다. 청크에 발화한 사람을
      // 기록해 두면, 그 사람에게는 자기 발화의 용어 카드를 보내지 않을 수 있다.
      const speakerUserId = String(segment?.userId || "").trim();
      if (speakerUserId) pending.userIds.add(speakerUserId);
