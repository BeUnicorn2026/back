...
export function speakerVerificationUpdate(speaker, evidence = {}) {
  ...
  if (!expectedSpeakerId) return { recorded: false, reason: "expected_not_selected", changes: null };
  if (!predictedSpeakerId) return { recorded: false, reason: "expected_not_matched", changes: null };
  if (predictedSpeakerId !== expectedSpeakerId) {
    return { recorded: false, reason: "unexpected_identity", changes: null };
  }
